...
> **Файлы:** `index.html`, `server.py`, `monitor.py`, `monitor.html`, `docs/system.md`
...
scheduleRender()
  → requestAnimationFrame()
    → renderAll()
      ...
      → renderVolumes()    — гистограмма объёмов (D3 .join())
      → renderCandles()    — японские свечи (D3 .join())
      → renderAxes()       — оси X/Y
      → renderIndicators() — SMA, BB, RSI
      → renderFractals()   — авто-уровни + подпись цены
      → renderRays()       — лучи (ховер, drag, dasharray)
      → renderRects()      — прямоугольники
      → renderTexts()      — текст
      → renderPriceLines() — гор. линии (ховер, drag)
...
- **Курсор** — режим по умолчанию, просмотр данных
- **Линия (горизонтальная)** — клик → пунктирная линия + метка цены справа
- **Луч** — один клик → линия от точки клика вправо. Двойной клик → панель настроек (цвет, толщина, сплошная/пунктир, текст метки, удаление). ПКМ + drag → перемещение. Включает магнит автоматически.
- **Прямоугольник** — клик A → drag → клик B → прямоугольник. ПКМ + drag → перемещение.
- **Линейка** — клик A → drag (превью разницы) → клик B → линия + бейдж (разница цены, %, бары). После фиксации переключается в курсор.
- **Текст** — клик → метка «Метка». Двойной клик → панель настроек (текст, цвет, размер, удаление). ПКМ + drag → перемещение.
- **Магнит** — привязка кроссхеира/кликов к OHLC свечи. Автоматически включается при рисовании лучей.
- **Ластик** — удаляет все нарисованные объекты (кроме авто-уровней).

### Взаимодействие с инструментами
- **Ховер**: при наведении на луч/линию/прямоугольник элемент подсвечивается (увеличение opacity/толщины)
- **Перетаскивание**: ПКМ (правая кнопка мыши) + удерживание + движение → перемещение инструмента по графику
- **Панель настроек**: двойной клик по инструменту → плавающая панель с настройками (перетаскивается за заголовок)
- **Палитра цветов**: в панели настроек луча/текста — 5 предустановленных цветов (золото, зелёный, красный, пурпурный, серый) + color picker
...
| # | Проблема | Статус |
|:---|:---|:---|
| 1 | OI5m всегда «—» (нет данных Open Interest) | 🟡 Известно |
| 2 | При клике на монету в правой панели активная строка не всегда переподсвечивается | 🟡 Известно |
| 3 | Загрузка NATR для ВСЕХ монет с фронтенда (медленно) → вынесено на сервер | ✅ Решено |
| 4 | WebSocket уходил в рекурсивный цикл переподключения | ✅ Решено (onclose = null) |
| 5 | Таймауты `/api/tickers` при большой нагрузке | ✅ Решено (микрокэш 1с) |
| 6 | CH24h мерцание (WS перезаписывало API данные) | ✅ Решено (приоритет WS) |
| 7 | «Застывшие свечи» — свечи не обновлялись при движении цены | ✅ Решено (D3 .join()) |
| 8 | Лучи от левого края SVG | ✅ Решено (один клик, от точки клика) |
| 9 | Нет инструментов: прямоугольник, текст | ✅ Решено (добавлены) |
...
- [x] Фикс CH24h мерцания (приоритет WS)
- [x] Фикс «застывших свечей» (D3 .join())
- [x] Инструмент «Луч»: один клик, от точки клика вправо
- [x] Инструмент «Прямоугольник»: два клика
- [x] Инструмент «Текст»: клик + панель настроек
- [x] Инструмент «Линейка»: TradingView-стиль + автовыключение
- [x] Панель настроек инструментов: двойной клик, перетаскивание
- [x] Палитра цветов (5 предустановленных + color picker)
- [x] Ховер-эффекты на инструментах
- [x] Перетаскивание инструментов (ПКМ + drag)
- [x] Сплошная/пунктирная линия для лучей
- [x] Магнит автоматически для лучей
- [x] Удалён тип графика «Линия» (только японские свечи)
- [x] Индикаторы: убраны чекбоксы, выделение цветом
- [x] Кроссхеир: свободное движение + метка времени на оси X
- [x] Ластик не удаляет авто-уровни (фракталы)
- [x] Подпись цены на авто-уровнях (фракталах)
- [x] Бот-тестировщик (monitor.py v2) + дашборд (monitor.html)
- [x] Документация (docs/system.md)
- [ ] OI5m — данные Open Interest
- [ ] Log-scale для оси Y
